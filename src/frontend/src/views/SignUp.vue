<template>
  ...
  <v-container class="grey lighten-5 container">
    <v-row>
      <v-col>
        <h2 class="header">{{ t("pages.signup.create_free_account") }}</h2>
      </v-col>
    </v-row>
  ...
  methods: {
    async handleSignUp() {
      try {
        ...
        const { first_name, last_name, email, accept_terms } = this.formData;
        const formData = {
          first_name,
          last_name,
          email,
        ...
        await createRegister(formData)
          ...
          .catch(() => {
            this.$refs.errorToast.showToastError("Please already exist");
          });
      } catch (error) {
